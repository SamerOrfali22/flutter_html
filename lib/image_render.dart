  ...
  Widget Function()? loadingWidget,
    ...
    (context, attributes, element) {
      ...
      if (context.parser.cachedImageSizes[src] != null) {
        ...
        final Image image = Image.network(src, frameBuilder: (ctx, child, frame, _) {
          if (frame == null) {
        ...
        listener = ImageStreamListener((ImageInfo imageInfo, bool synchronousCall) {
          ...
          final Size size = Size(myImage.width.toDouble(), myImage.height.toDouble());
          if (!completer.isCompleted) {
            context.parser.cachedImageSizes[src] = size;
            completer.complete(size);
            image.image.resolve(ImageConfiguration.empty).removeListener(listener!);
          }
